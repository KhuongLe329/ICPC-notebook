...
public:
    ...
    static void poly_square_split(ivec& vout, ivec const&v1, const size_t mod = 1e9 + 7){
        static cvec c1, c2;
...
void add(ll& x, int y, int m = mod) {if((x += y) >= m) x -= m;}
void sub(ll& x, int y, int m = mod) {if((x -= y) < 0) x += m;}
int mul(int x, int y, int m = mod) {return 1ll * x * y % m;}

int fac[N], inv[N], n, x, y, r, t, k, l, p;
int sum[2], sz[2], cnt[2][12];
Polynomial<mod> fa, fb, fc;
typedef vector<ll> vi;

int nCk(int n, int k) {return 1ll * fac[n] * inv[k] % mod * inv[n - k] % mod;}
int Pow(int x, int y) {
    int res = 1;
    for(; y > 0; y >>= 1, x = mul(x, x)) if(y & 1) res = mul(res, x);
    return res;
}

vi Mul(vi a, vi b) {
   fa.clear(), fb.clear(); fa.resize(a.size()), fb.resize(b.size());
...
vi DP(int* cnt) {
    ...
    for(int i = 0; i < 11; ++i) {
        ni = cnt[i];
        vi tmp((ni + 1) * 121);
        for(int j = 0; j <= ni; ++j) tmp[j * 121 + i * j % 11] = nCk(ni, j);
        res = Mul(res, tmp);
    }
    return res;
}
...
void Solve() {
    cin >> n;
    for(int i = 0; i < 11; ++i) cnt[0][i] = cnt[1][i] = 0;
    sum[0] = sum[1] = sz[0] = sz[1] = 0;
    for(int i = 1; i <= n; ++i) {
        cin >> x; y = x % 11;
        while(x >= 100) x /= 100;
        if(x < 10) ++cnt[0][y], sum[0] = (sum[0] + y) % 11, ++sz[0];
        else ++cnt[1][y], sum[1] = (sum[1] + y) % 11, ++sz[1];
    }
    odd = DP(cnt[0]), even = DP(cnt[1]);
    k = sz[0] + 1 >> 1, l = sz[0] >> 1, p = sz[0] + 2 >> 1;
    vi vc(11);
    for(int i = 0; i < int(even.size()); ++i) if (even[i]) {
        x = i / 121, y = sz[1] - x;
        r = (2 * i - sum[1] + 11) % 11, t = 1;
        if (x) t = mul(t, mul(nCk(p + x - 1, x), fac[x]));
        if (y) t = mul(t, mul(nCk(k + y - 1, y), fac[y]));
        t = mul(t, even[i]);
        add(vc[r], t);
    }
    ll res = 0;
    for(int i = 0; i < int(odd.size()); ++i) {
        x = i / 121;
        y = (11 - (2 * i - sum[0]) % 11 + 11) % 11;
        if (x == k) {
            t = mul(mul(mul(vc[y], odd[i]), fac[k]), fac[l]);
            add(res, t);
        }
    }
    cout << res << '\n';
